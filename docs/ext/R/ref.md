...
results from a particular strata, e.g. for [`PSD`](../../ref/power-spectra.md#psd) results defined by
...
!!! note
    ...
    the second [`DESC`](../../ref/summaries.md#desc) command:
    ...
    In R, with the same EDF [`lattach()`](#lattach()-ed, the second [`DESC`](../../ref/summaries.md#desc) statement 
    ...
    leval( "DESC" ) 
    ```
    That is, even though separate `leval()` statements are made, they are really equivalent to the following command line (i.e. which would also show that only 1 signal is present from [`DESC`](../../ref/summaries.md#desc)):
    ```
...
For correspondence with Luna's [`SIGSTATS`](../../ref/summaries.md#sigstats), the above function also
...
epoch-level output from the actual [`SIGSTATS`](../../ref/summaries.md#sigstats) command, which
...
for the [`EPOCH`](../../ref/epochs.md#epoch) command (which only has a single strata/data-frame),
...
In contrast, the [`PSD`](../../ref/power-spectra.md#psd) command has four associated strata/data-frames
...
(as in the [`PSD`](../../ref/power-spectra.md#psd) example), we specify an additional set of arguments
...
!!! hint 
    ...
    1-item list containing a data-frame).  For example, if [`EPOCH`](../../ref/epochs.md#epoch)
...
to the _command_ (e.g. [`PSD`](../../ref/power-spectra.md#psd)) followed by any stratifying _factors_
(e.g. `B,CH`).
...
used by [`PSD`](../../ref/power-spectra.md#psd) (`B` equals `SLOW`, `DELTA`, `THETA`, `ALPHA`, `SIGMA`,
