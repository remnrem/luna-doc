...
_folder_, which will be searched for matching [FTR
files](../ref/annotations.md#ftr-files): e.g.
...
`.annot`, `.eannot` and `.ftr`.  You can specify a comma-delimited list of multiple extensions. 
...
in that folder (although see notes about [FTR
files](../ref/annotations.md#ftr-files)).  Typically, one might
...
| [.annot](../ref/annotations.md#annot-files) | Generic Luna annotation files | 
...
| [FTR](../ref/annotations.md#ftr-files) | _Feature_ files, typically generated by certain Luna commands | 
...
In a few contexts (various outputs and FTR files, for example) Luna
encodes time in _time-points_ where 1 unit is 10<sup>-9</sup> seconds
...
| [`annot-folder`](#annotations)| Project-wide folder to look for FTR annotation files in |
...
| [`ftr`](#annotations) | Y/N to load FTR files automatically (default Y) |
...
The `annot-folder` option specifies a single, project-wide annotation
folder, which will be searched for FTR files that belong to that
individual:
...
The `ftr` option (yes/no) turns on or off loading of FTR files
