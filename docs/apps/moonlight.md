...
running [`MASK`](../ref/masks.md#mask) via the _Luna_ panel), this button will effectively run
a [`RESTRUCTURE`](../ref/masks.md#restructure) command, to actually remove those masked epochs from
...
[`PSD`](../ref/power-spectra.md#psd) command.  The x-axis is all currently included epochs; the
...
[`RESTRUCTURE`](../ref/masks.md#restructure)) will remove gray epochs (and plot them as white,
...
[`SEGMENTS`](../ref/outputs.md#segments) command and graphs the output.  If epochs have subsequently
...
Basic summaries from Luna's [`HYPNO`](../ref/hypnograms.md#hypno) command, including TST, WASO, sleep efficiency, etc.

### Times

Key times from [`HYPNO`](../ref/hypnograms.md#hypno) annotated in elapsed seconds, epochs and clock-time:
...
estimates of stage duration, all as generated by the [`SOAP`](../ref/soap.md#soap) command.
...
see a power spectra emitted from the previous command, select `F` for the _X-axis_, and [`PSD`](../ref/power-spectra.md#psd) for the _Y-axis_.  You should then see the following
plots:
