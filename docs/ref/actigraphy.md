...
These commands support analysis of wrist or waist actigraphy recordings spanning multiple days. [`DAYS`](actigraphy.md#days) creates structured day and clock-time annotations (calendar days, hourly windows, and AM/PM halves) across the recording, providing the temporal scaffolding needed for circadian analyses. [`ACTIG`](actigraphy.md#actig) computes non-parametric circadian rhythm metrics (IS, IV, RA, L5/M10, and related measures) and applies a simple threshold-based wake/sleep scoring algorithm to the actigraphy signal.
...
[`DAYS`](actigraphy.md#days) creates annotation intervals marking calendar days and optionally hourly
...
[`DAYS`](actigraphy.md#days) requires a valid EDF start date and time. Beginning from the first anchor-hour boundary at or after recording onset, successive 24-hour windows are placed end-to-end across the full recording duration, each labeled with a zero-padded day index (e.g., `day01`, `day02`). The initial partial window — spanning from recording start to the first anchor boundary — is emitted as a separate annotation, so that all subsequent windows are anchored to the exact same clock hour. Hourly annotations, when requested, are generated by iterating through every clock hour of the recording and emitting one-hour-long intervals labeled with their zero-padded hour of day (`00h`–`23h`). AM and PM annotations span midnight-to-noon and noon-to-midnight, respectively, tiled across the full recording. Weekend annotations are determined from the calendar date of each anchored day boundary. All annotations are written directly to Luna's in-memory annotation store.
...
!!! warning "Active development"
    [`ACTIG`](actigraphy.md#actig) is under active development and expansion. Defaults, scoring
    options, QC behavior, and output details may still evolve.

[`ACTIG`](actigraphy.md#actig) analyses a continuously sampled activity channel — actigraphy counts,
...
[`ACTIG`](actigraphy.md#actig) bins the activity signal into fixed-length non-overlapping epochs anchored to recording time; epochs with fewer than a minimum percentage of their expected samples are flagged as gaps and excluded from all downstream calculations. Non-parametric circadian rhythm metrics — Interdaily Stability (IS), Intradaily Variability (IV), Relative Amplitude (RA), and the least-active (L5) and most-active (M10) window means and onset times — are computed from a 24-hour mean activity profile constructed by averaging epochs falling within each clock-time bin across all recording days. L5 and M10 onsets are identified by a fine-step sliding-window search over the circular 24-hour profile. Sleep/wake scoring with the default `luna` method proceeds in three stages applied within each contiguous non-gap block: (1) the raw activity values are log-transformed and robustly normalized using the recording-wide median and median absolute deviation; (2) a centred moving-median filter produces a smoothed z-score track, and a local burst-density estimate quantifies the fraction of surrounding epochs exceeding a burst z-score threshold; (3) an epoch is classified as candidate sleep if its smoothed z-score is below `quiet-z` and its burst density is below `active-frac`, after which sequential passes refine the binary label by filling brief wake gaps, removing short sleep runs, and terminating sleep bouts at sustained wake. The Cole–Kripke method applies the published 1992 weighted moving-average algorithm to 60-second activity counts. Fragmentation metrics — including sleep-to-wake transition probabilities, Shannon conditional transition entropy, and bout-duration percentiles — are computed separately within the sleep-period and wake-period windows. Day-level QC flags days with flatline signal, collapsed variability, or biologically implausible sleep fractions; days accumulating two or more technical flags are excluded from cross-day averages.
...
discontinuities or intervals masked beforehand with [`MASK`](masks.md#mask) and `RE`.
...
!!! note "EDF+D holes vs within-record gaps"
    After [`MASK`](masks.md#mask) + `RE`, the EDF becomes discontinuous (EDF+D). ACTIG detects
    ...
    [`MASK`](masks.md#mask). This ensures `TST + TWT + GAP_MIN = valid retained-record minutes`
...
| [`GP`](cc.md#gp) | Gap run | `score-period` | Gap epochs within period scoring |
...
| **prescored (implicit)** | [`ACTIG`](actigraphy.md#actig) (S/W exist) | No | Automatically detects S/W in memory and proceeds as prescored |
...
| `gap-out` | | `G` / [`GP`](cc.md#gp) | Output label for gap runs |
