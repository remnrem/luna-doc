...
particular example, there happens to be only a single EDF).  We also see a
...
plot( d$E , d$RMS , col = d$ID , pch=20 , xlab = "Epoch" , ylab = "RMS(ECG)" )
```
...
| `flat`    | 1e-6  | Report proportion of time with a flat signal (default 1e-6 if not specified) |
| `clipped` |       | Report proportion of time with a clipped signal |
...
Finally, to interpret the `POSITION` signal: if the values correspond to categories, we naturally need external
...
For this type of signal, it can often be convenient to create a corresponding interval-based annotation also,
