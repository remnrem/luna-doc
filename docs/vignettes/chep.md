...
| --------------------------------------------------------------- |
| <http://zzz-luna.org/dist/luna/chep-vig.zip>             |
...
([`CORREL`](../ref/cc.md#correl)), low predictability of a channel based on other channels
...
* [`CLOCS`](../ref/spatial.md#clocs) - to specify channel locations, which are needed by [`INTERPOLATE`](../ref/spatial.md#interpolate)
...
bad channels using the `CHEP-MASK` and [`CHEP`](../ref/masks.md#chep) commands.  In brief:
...
most Luna commands (e.g. [`PSD`](../ref/power-spectra.md#psd), etc), although there are a few
exceptions: [`CHEP`](../ref/masks.md#chep) and [`INTERPOLATE`](../ref/spatial.md#interpolate) being the primary ones that are
...
of the [`CHEP`](../ref/masks.md#chep) command is to translate between this internal _CHEP_
...
defined as outliers.  Further, we'll use [`CHEP`](../ref/masks.md#chep) to take the
...
been flagged as outliers. To do this, [`CHEP`](../ref/masks.md#chep) is called with the
...
!!! Note "Other CHEP functionality"
    ...
    [`CHEP`](../ref/masks.md#chep) such as loading, saving or clearing CHEP masks, masking epochs,
    etc.
...
The plot above is a direct illustration of how the [`CHEP`](../ref/masks.md#chep) mask works
...
!!! Note
    The [`INTERPOLATE`](../ref/spatial.md#interpolate) command needs information about channel locations, which can be attached via the [`CLOCS` command](../ref/spatial.md#clocs).


Now we will proceed to interpolate bad channels.  As well as channel
locations, the [`INTERPOLATE`](../ref/spatial.md#interpolate) command requires a valid _CHEP_ mask to
...
First, we will re-run the `CHEP-MASK` and [`CHEP`](../ref/masks.md#chep) commands to generate a file that contains
...
Now we will use the text file we just saved from [`CHEP`](../ref/masks.md#chep) and reload it
to define the _CHEP_ mask.  (As noted above, we could simply have
re-run the `CHEP-MASK` and [`CHEP`](../ref/masks.md#chep) commands in this second run rather
...
[`SIGSTATS`](../ref/summaries.md#sigstats) and [`PSD`](../ref/power-spectra.md#psd) once more to make sure that interpolated channels
...
!!! hint "Dropping channels instead of interpolating them"
    If you wished simply to remove the bad channels (instead of interpolating them), then replace [`INTERPOLATE`](../ref/spatial.md#interpolate)
    with [`RESTRUCTURE`](../ref/masks.md#restructure).

!!! hint "Specifying channel labels"
    By default, Luna will feed all data channels into commands such as `CHEP-MASK`, [`CHEP`](../ref/masks.md#chep), [`INTERPOLATE`](../ref/spatial.md#interpolate), etc.  If the EDF also contains non-EEG channels, or you only want a subset of the EEG
    channels to be considered, then add a `sig` option to any of these commands.  For example, if specified, the [`CHEP`](../ref/masks.md#chep) command uses the number of channels specified in `sig` as the denominator
...
[`INTERPOLATE`](../ref/spatial.md#interpolate) command works on an epoch-by-epoch basis, where the
...
defined as outliers when we first ran `CHEP-MASK` and [`CHEP`](../ref/masks.md#chep) commands (rather than
extracting only a handful of bad channels).  The primary difference is that we do not
add the `black-and-white` option for the [`CHEP`](../ref/masks.md#chep) command:
...
!!! hint "Saving cleaned EDFs"
    ...
    interpolation once but then typically just generated a [`PSD`](../ref/power-spectra.md#psd) or
...
channel-agnostic epoch-level mask pairs with the [`RESTRUCTURE`](../ref/masks.md#restructure)
...
!!! Note "Using [`TAG`](../ref/summaries.md#tag)s to track output"
    This time we will dump the [`CHEP`](../ref/masks.md#chep) mask only from the second time we run
    [`CHEP`](../ref/masks.md#chep) command.  If one wanted to output the same measures from two or more
