...
As described [here](../ref/ms.md$ms), segmentation of EEG microstates
...
the whole recording, or only the dats points that correspond to the
...
majority of studies report 4 canonical microstates.  You should select
...
Both Luna and Cartool have various other options, and Cartool offers
other features that may be particularly useful in other contexts. That
...
There are different ways to study the transitions of microstates. For example, the previous
command outputs posterior probabilities of moving to state _J_ given you are currently in state _I_:
```
destrat fit.db -r PRE -c POST
```
...
enriched or depleted compared chance expectation.  This is done by
shuffling the states in a way that preserves the overall frequnecies
...
in this vignette the subjects in each group are infact identical copies of each other, including here only to illustrate how these
commands would be applied in the context of real data.)

```
echo "file=states.txt vars=grouping.txt phe=PHE k1=4 k2=4 nreps=100" | luna --kmer -o kmer.db
```

One common use case is to code the control group as 0 and the case group as 1.  Specfically:

 - `file` points to the aggregated set of states for all six individuals

 - `vars` points to the file with the group/phenotype (`PHE`), which
   is indexed by a column `ID` that should match the EDF IDs
...
```
destrat kmer.db +KMER -r L S 
```

we see that, for example, `ABAB` is enriched (Z = 8.2, p < 0.01):
whereas we'd expect a rate of occurrence of 0.016 (`EXP`), the
observed rate was `0.022` (`OBS`), an increase of 1.35-fold (`RAT`).
...
also shows this group is enriched. In contrast, the equivlance group
`ABCD` (i.e any k-mer that has all four states, in any order) appears
under-represented, Z = -11 (expected 0.205, observed 0.176).
...
regarding the enrich/depletion of kmers in each group and the
...
Finally, rather than fitting a single segmentation/clustering to peaks
aggregated across all individuals (i.e. and so forcing a single
solution to be applied to all subjects), we can perform the
segmentation separately and ask whether or not the maps generated (for
...
with the prior steps of first creating an aggregated `peaks.edf` and
then separately back-fitting.  There is just a single command:
...
at random for the segemntation for that individual.


Although in this toy example, subjects 1-3 and subjects 4-6 are infact
...
We can extract the maps (`A` matrix) for each individual as follows (here `KN` is always 4, as we only ran a _K=4_ job):
...
How question is how to quantify whether these solutions are "similar"
...
that maximises the total correlation: in this example it should be:
...
A, B, C and D as defined by the literature). The global similarity measure is then
mean of these 4 spatial correlations _S_ = ( 0.937 + 0.928 +  0.892 + 0.688 ) / 4 .
...
using `vars` and `phe` to specifying the binary partitioning.  Here
...
enables automated analysis for large scale of data set. Based on our
