...
using `SET-HEADERS` followed by [`WRITE`](../ref/outputs.md#write) to make a new EDF:
...
(Note that the end time given by [`DESC`](../ref/summaries.md#desc) is one time point past the end
...
relative to the header start time.   Now when we look at the [`DESC`](../ref/summaries.md#desc), Luna will give some extra information describing the
EDF+D features:
```
luna merged.edf -s DESC
```
...
Alternatively, we can use [LunaScope](https://zzz-luna.org/lunascope/) to view the structure of an
...
If we upload both `merged.edf` and `merged.annot` to [LunaScope](https://zzz-luna.org/lunascope/), we can now see the annotations are appropriately aligned
...
Here we use the simple [`SIGGEN`](../ref/simul.md#siggen) command to simulate a 30-second dataset with
...
generated by the [`MATRIX`](../ref/outputs.md#matrix) command:
...
```
and from [`WRITE`](../ref/outputs.md#write):
```
  data are truly discontinuous
  writing 2 channels
...
The [`RECS`](../ref/outputs.md#recs) command dumps the EDF record structure/times to the console
...
Finally, it is also possible to use the [`SEGMENTS`](../ref/outputs.md#segments) command with an EDF+D (whether a true EDF+D file, or
an in-memory EDF+D, i.e constructed after setting masks and calling [`RESTRUCTURE`](../ref/masks.md#restructure)) to generate new annotations
...
!!! hint
    As above, some commands implicitly use epochs to iterate over data, including the [`ANNOTS`](../ref/annotations.md#annots) command.  Because we are working with
