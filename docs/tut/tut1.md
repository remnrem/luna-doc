...
correctly, run the following to apply the [`DESC`](../ref/summaries.md#desc) _command_ to each EDF
...
`cat` or `less`), we see the description generated by the [`DESC`](../ref/summaries.md#desc)
command:
...
10 or 11 hours of sleep.  Much of the output of [`DESC`](../ref/summaries.md#desc) mirrors what is
...
Turning to the signals contained in each EDF, here we use the [`STATS`](../ref/summaries.md#stats)
...
command, one advantage of [`ANNOTS`](../ref/annotations.md#annots) is that other _masks_ can
be applied: for example, to count only apneas occurring during REM
sleep.  Running [`ANNOTS`](../ref/annotations.md#annots) and sending output to `annot.db`:
...
By default, the [`ANNOTS`](../ref/annotations.md#annots) command will include all annotations with
...
We'll end this first section by combining the [`STATS`](../ref/summaries.md#stats) command with the
...
First, [`EPOCH`](../ref/epochs.md#epoch) specifies that non-overlapping, 30-second epochs
should be applied to each EDF.  We then set a [`TAG`](../ref/summaries.md#tag), which helps
...
one or more epochs, the [`RESTRUCTURE`](../ref/masks.md#restructure) command removes any masked
epochs. Finally, the [`STATS`](../ref/summaries.md#stats) command is invoked, but this time it
...
[`TAG`](../ref/summaries.md#tag) and [`MASK`](../ref/masks.md#mask) commands. Finally, we set a special variable,
`sig`, which instructs Luna to only consider the first EEG
channel (labeled `EEG`, as indicated by the [`DESC`](../ref/summaries.md#desc) command):
...
The [`TAG`](../ref/summaries.md#tag) command specified that a user-defined stratum, called
[`STAGE`](../ref/hypnograms.md#stage), be added to the output.  This was set to either `N1`,
`N2`, etc, corresponding to the sleep stage values in the XML
file.  There are therefore 4 _levels_ for the _factor_
[`STAGE`](../ref/hypnograms.md#stage). We can view the RMS statistics, which are grouped by
channel (`CH`) and sleep stage ([`STAGE`](../ref/hypnograms.md#stage)), as follows:
...
output after any [`RESTRUCTURE`](../ref/masks.md#restructure) command: `DUR2` is the duration in
...
What about the above [`THAW`](../ref/freezes.md#thaw) output noting 15 versus 14 signals, given
