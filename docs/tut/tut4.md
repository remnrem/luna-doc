...
Docker will look for an image called `remnrem/lunars` and if it is not
...
```
Unable to find image 'remnrem/lunars:latest' locally
latest: Pulling from remnrem/lunars
...
Login with user-name `rstudio` and the password you specified
above (i.e. `abc123` in this example):
...
FTR, `.annot` or `.eannot` files.  One option would be to save the
